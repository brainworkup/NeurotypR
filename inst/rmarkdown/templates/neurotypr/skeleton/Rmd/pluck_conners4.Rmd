...
# file <- "/Users/joey/reports/Biggie/data/pdf/Harrison_Lucas_Conners4_Single-RaterReport_Parent.pdf"
...
areas_1 <- bwu::gpluck_locate_areas(
  file = file,
  pages = pages1
)
```

```{r locate-areas2, eval=FALSE}
areas_2 <- bwu::gpluck_locate_areas(
  file = file,
  pages = pages2
)
```
...
save_area <- function(tbl, i, report_type, test) {
  ...
  file_name <- paste0("area_table_", test, "_", i, "_conners4_", report_type, ".rds")
  saveRDS(tbl, file = file_name)
}

report_type <- "parent"
# Use map2 to iterate over the tables and their indices
map2(areas, seq_along(areas), save_area, report_type = report_type, test = test)
```
...
merge_and_keep_rows <- function(tbl) {
  ...
  result <- tibble(
    ...
    ci_95 = df[[2, 4]],
    percentile = df[[2, 5]],
    category = "Slightly Elevated", # this will need to be changed
    diff = df[[2, 7]],
    significance = df[[2, 8]]
  )
...
colnames(df1) <- c("scale", "raw_score", "score", "ci_95", "percentile", "category", "diff", "significance")
...
if (params$test == "conners4_parent") {
  df_mutated <-
    ...
    dplyr::mutate(
      result = dplyr::case_when(
        scale == "Inattention/Executive Dysfunction" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "Hyperactivity" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "Impulsivity" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "Emotional Dysregulation" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "Depressed Mood" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "Anxious Thoughts" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
        ...
        scale == "Total ADHD Symptoms" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "Oppositional Defiant Disorder Symptoms" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "Conduct Disorder Symptoms" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "ADHD Index" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) indicated a probability of {probability_score} of having ADHD, which in this context is considered to be {range} at risk for ADHD .\n\n"),
        scale == "Schoolwork" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "Peer Interactions" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "Family Life" ~ glue::glue("- Parent-reported {scale} (i.e., {description}) was {range}.\n\n"),
  ...
  df_mutated <-
    ...
    dplyr::mutate(
      result = dplyr::case_when(
        scale == "Inattention/Executive Dysfunction" ~ glue::glue("- Self-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "Hyperactivity" ~ glue::glue("- Self-reported {scale} (i.e., {description}) was {range}.\n\n"),
        scale == "Impulsivity" ~ glue::glue("- Self-reported {scale} (i.e., {description}) was {range}.\n\n"),
