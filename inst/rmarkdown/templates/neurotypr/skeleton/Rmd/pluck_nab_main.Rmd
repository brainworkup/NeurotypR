...
table4 <-
  ...
  dplyr::mutate(
    narrow = dplyr::case_when(
      grepl("List Learning Immediate", scale) ~ "Word-List Learning",
      grepl("List Learning Delayed", scale) ~ "Free-Recall Memory",
      TRUE ~ as.character(narrow)
    )
  )
...
table7 <-
  ...
  dplyr::mutate(
    narrow = dplyr::case_when(
      ...
      grepl("Daily Living Memory Recall", scale) ~ "Everyday Memory",
      TRUE ~ as.character(narrow)
    )
  )
...
table2 <-
  ...
  dplyr::mutate(
    pass = dplyr::case_when(
      ...
      grepl(dsf, scale) ~ "Sequential",
      grepl(dsb, scale) ~ "Attention",
      grepl(c("Part A Efficiency", "Part A Speed"), scale) ~ "Processing Speed",
      grepl(c("Part B Efficiency", "Part C Efficiency", "Part D Efficiency"), scale) ~ "Attention",
      TRUE ~ as.character(pass)
    )
  )
...
# readr::write_excel_csv(df, file_path, append = TRUE, na = "")
```
