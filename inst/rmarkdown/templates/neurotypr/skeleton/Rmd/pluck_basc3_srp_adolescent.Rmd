---
title: |
  <center> BASC-3 SRP Adolescent </center>
date: "`r Sys.Date()`"
params:
  patient: Biggie
  test: basc3_srp_adolescent
  test_name: "BASC-3 SRP Adolescent"
  file:
...
# SRP
areas <- list(
  table1 = c(173, 88, 240, 520),
  table2 = c(413, 36, 622, 575)
)
```
...
extracted_tables <- tabulapdf::extract_tables(
  ...
  area = areas,
  guess = NULL,
  method = "stream", # stream, lattice, decide
  output = "matrix"
)
...
column_names2 <- c("scale", "raw_score", "score", "percentile", "ci_95", "difference", "sig_level") # frequency doesnt extract if no data
...
if (params$test == "basc3_srp_adolescent") {
  ## extract text
  basc3_srp_adolescent_txt <- get_text(file)
  if (!is.null(basc3_srp_adolescent_txt)) {
    # Replace long spaces with a column break symbol
    basc3_srp_adolescent_squished <- stringr::str_replace_all(basc3_srp_adolescent_txt, "\\s{2,}", "- ") |>
      stringr::str_remove_all(",")

    # save as markdown
    readr::write_lines(
      basc3_srp_adolescent_squished,
      here::here("basc3_srp_adolescent_text.md"),
      sep = "\n"
    )
  }
}
