#' Extract and Process Neuropsychological Test Data from PDF Files
#'
...
extract_test_data <- function(
  ...
  if (write_output) {
    ...
    save(extracted_areas, file = file.path(output_dir, paste0(test, "_extracted_areas.rds")))
  }
  ...
  if (write_output) {
    ...
    readr::write_excel_csv(df, file.path(output_dir, paste0(test, ".csv")), col_names = TRUE)
  }
...
#' Function to merge multiple CSV files from the same test
#'
#' @param test Character string with the test prefix for files to merge
#' @param suffix Character string with the file suffix (default: "csv")
#' @param directory Character string with the directory containing the files
#'
#' @return A data frame containing the merged data
#' @export
merge_subtests <- function(test, suffix = "csv", directory = here::here("data", "csv")) {
  # Get the list of files matching the prefix and suffix
  files <- dir(directory, pattern = paste0(test, "_[0-9]+\\.", suffix), full.names = TRUE)

  # If no files are found, return an empty data frame
  if (length(files) == 0) {
    warning("No files found matching pattern: ", paste0(test, "_[0-9]+\\.", suffix))
    return(data.frame())
  }
